...
class Application
{
	...
	std::vector<vector3> m_shapeCurrentPositions; //vectors for current circle positions
	std::vector<vector3> m_shapeEndPositions; //vectors for position each circle is seeking
	std::vector<float> m_angles;	//floats of angles for each position
	float m_stepAngle;	//float of the increment for each angle
...
#pragma region Initialization / Release
	...
	*/
	void Release(void);
	/*
	USAGE: Determines the next stop for a circle to make after it reaches its destination
	ARGUMENTS: numSides = the number of sides this circle's torus has
			   radius = the radius of the torus
	OUTPUT: a vector3 for the circle's next position to lerp to
	*/
	vector3 CalculateNextStop(int numSides, float radius);
#pragma endregion
