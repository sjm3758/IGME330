...
class Application
{
	uint m_uMeshCount = 58;
	std::vector<MyMesh*> cubes;
	String m_sProgrammer = "Sean Mack - sjm3758@rit.edu";
...
#pragma region Main Loop
	...
	*/
	...
	/*
	USAGE: Executes the Render function with the mesh's
	specific model
	ARGUMENTS: int currentMesh -> the model in the cubes vector being rendered
	matrix4 m4Translate -> Specific mesh's position in the world space
	matrix4 m4Scale -> Specific mesh's model
	OUTPUT: ---
	*/
	void ModelRender(int currentMesh, matrix4 m4Translate, matrix4 m4Scale);
	/*
