/*----------------------------------------------
Programmer: Sean Mack (sjm3758@rit.edu)
Date: 2018/11
----------------------------------------------*/
...
namespace Simplex
{
	...
	class MyOctant
	{
		...
		MyRigidBody* m_pRigidBody = nullptr; //Rigid body to test for collisions
		std::vector<MyOctant*> m_lChild; //list of nodes that contain objects (this will be applied to root only)

		std::vector<vector3> m_v3Corners; //corners of the octant to make a rigid body and test for collisions
	...
	public:
		...
		*/
		...
		OUTPUT: class object
		*/
		MyOctant(vector3 a_v3Center, float a_fSize, uint a_uCurrentLevel, MyOctant* parent, uint id);
		/*
	...
	private:
		...
		*/
		...
		OUTPUT: ---
		*/
		void Init(uint a_uCurrentLevel, uint count);
		/*
