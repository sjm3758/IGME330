...
void Application::Display(void)
{
	...
	vector3 endPos;
	static uint route = 0;
	startPos = m_stopsList[route];
	endPos = m_stopsList[(route + 1) % m_stopsList.size()];
	float timeBetweenStops = 1.0;//in seconds
	float percentage = MapValue(fTimer, 0.0f, timeBetweenStops, 0.0f, 1.0f);
	v3CurrentPos = glm::lerp(startPos, endPos, percentage);
	if (percentage >= 1.0f)
	{
		route++; 
