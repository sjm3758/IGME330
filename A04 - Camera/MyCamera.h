...
public:
	...
	*/
	...
	/*
	USAGE: Rotates the camera based on the mouse's offset from the middle of the screen. (Hold the right mouse button and drag)
	ARGUMENTS: 
	 -	float xAngle -> mouse's X offset
	 -	float yAngle -> mouse's Y offset
	OUTPUT: ---
	*/
	void ChangeCameraVectors(float xAngle, float yAngle);
	/*
	USAGE: Calculates the forward, rightward, and upward vectors in camera space
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	void CalculateForwardRightwardUpward();
};
