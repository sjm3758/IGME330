...
void Simplex::MyCamera::CalculateViewMatrix(void)
{
	...
	CalculateForwardRightwardUpward();
...
//moves the camera forward and backward based on camera space, then recalculates the forward, backward, and upward
...
//strafes the camera left and right based on camera space, then recalculates the forward, backward, and upward
...
//changes the camera's orientation based on the mouse's offset from the middle of the screen by setting the camera's target
void MyCamera::ChangeCameraVectors(float xAngle, float yAngle) 
{
	quaternion rotationX, rotationY, orientation;
	rotationX = glm::angleAxis(glm::radians(-xAngle), m_v3Rightward);
	rotationY = glm::angleAxis(glm::radians(yAngle), m_v3Upward);
	orientation = rotationY * rotationX;
	SetTarget(m_v3Target * orientation);
}

//recalculates the camera's forward, rightward, and upward vectors
void MyCamera::CalculateForwardRightwardUpward() {
	m_v3Forward = glm::normalize(m_v3Target - m_v3Position);
